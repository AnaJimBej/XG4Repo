...
class KnowledgeGraph(object):
    def __init__(self, data_path):
        ...
        # # Check shuffled relations
        # shuffled_keys = list(self.id2relation.keys())
        # random.Random(2).shuffle(shuffled_keys)
        #
        # new_id2relation = {}
        # new_relation2id = {}
        # for i in range(0,len(self.relation2id)):
        #     new_id2relation[i] = self.id2relation[shuffled_keys[i]]
        #     new_relation2id[self.id2relation[shuffled_keys[i]]] = i
        #
        # self.id2relation = new_id2relation
        ...
        with open(os.path.join(data_path, "graph.txt")) as fi:
            for line in fi:
        ...
        with open(os.path.join(data_path, "test.txt")) as fi:
            for line in fi:
                ...
                if hr_index not in self.hr2o:          # Include this to get a proper mask
                    self.hr2o[hr_index] = list()
...
class ValidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_unused = torch.ones(len(data), self.graph.entity_size).bool()
        for k, (h, r, t) in enumerate(data):
            hr_index = self.graph.encode_hr(h, r)
            t_index = torch.LongTensor(self.graph.hr2oo[hr_index])
            mask[k][t_index] = 0

        return all_h, all_r, all_t, mask, mask_unused
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_metric = torch.ones(len(data), self.graph.entity_size).bool()
        mask_list = torch.ones(len(data), self.graph.entity_size).bool()
        for k, (h, r, t) in enumerate(data):
            hr_index = self.graph.encode_hr(h, r)

            t_index_metric = torch.LongTensor(self.graph.hr2ooo[hr_index]) # mask includes train, validation, test
            t_index_list = torch.LongTensor(self.graph.hr2o[hr_index]) # mask only includes train
            mask_metric[k][t_index_metric] = 0
            mask_list[k][t_index_list] = 0

        return all_h, all_r, all_t, mask_metric, mask_list
...
class RuleDataset(Dataset):
    def __init__(self, num_relations, input):
        ...
        if type(input) == list:
            ...
            with open(input, 'r') as fi:
                for line in fi:
                    ...
                    #rule = [int(_) for _ in rule[0:-1]] + [float(rule[-1])]# MaaaaaaaaaaaaL
                    rules.append(rule)
        ...
        for rule in rules:
            ...
            #formatted_rule = [rule[0:-1] + [self.ending_idx], self.padding_idx, rule[-1]] # MaaaaaaaaaaaaL
